...
Default behavior:
  - macOS: use local HTTP bridge + launchd (recommended)
...
def resolve_transport(requested: str | None) -> str:
    if requested:
        return requested
    return "http" if is_mac() else "stdio"


def build_stdio_block(server_name: str, python_path: Path, server_path: Path, api_path: str | None, lib_path: str | None) -> str:
    ...
    for key in ("RESOLVE_SCRIPT_API", "RESOLVE_SCRIPT_LIB", "PYTHONPATH", "PYTHONHOME", "PATH"):
        value = env.get(key)
        if value:
...
def build_http_block(server_name: str, host: str, port: int) -> str:
    return "\n".join(
        [
            f"[mcp_servers.{server_name}]",
...
def install_launch_agent(
    ...
    plist_text = build_launch_agent_plist(
        python_path, server_path, project_dir, api_path, lib_path, host, port
    )

    if dry_run:
        print(f"\n  {cyan('LaunchAgent preview')} → {plist_path}")
        print(f"    label: {LAUNCH_AGENT_LABEL}")
        print(f"    command: {python_path} {server_path} --transport http --host {host} --port {port}")
        return
...
def ensure_python_env(project_dir: Path, no_venv: bool, python_override: str | None, dry_run: bool) -> Path:
    if python_override:
...
def verify_http_endpoint(host: str, port: int) -> tuple[bool, str]:
    try:
        result = subprocess.run(
            ["curl", "-sS", "-i", f"http://{host}:{port}/"],
            capture_output=True,
            text=True,
            timeout=5,
            check=False,
    ...
    if "406 Not Acceptable" in output:
        return True, "HTTP bridge is responding"
    return False, output.strip() or f"curl exit code {result.returncode}"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=("stdio", "http"),
        default=None,
        help="Codex transport to configure. Defaults to http on macOS, stdio elsewhere.",
    )
    parser.add_argument(
        "--host",
        default=DEFAULT_HTTP_HOST,
        help="HTTP bind host for Codex local bridge.",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=DEFAULT_HTTP_PORT,
        help="HTTP bind port for Codex local bridge.",
    )
    parser.add_argument(
        "--server-name",
    ...
    if not api_path:
        print(f"\n  {yellow('Warning:')} Resolve scripting API path was not detected.")
        if not args.dry_run and not prompt_yes_no("Continue anyway?", default=False):
            sys.exit(1)

    if transport == "http":
        block = build_http_block(args.server_name, args.host, args.port)
    else:
    ...
    if transport == "http":
        ok, message = verify_http_endpoint(args.host, args.port)
    ...
    if transport == "http":
        ...
        print("    2. Keep DaVinci Resolve Studio installed and External scripting set to Local")
        print("    3. Ask Codex to list Resolve tools or read the current project name")
    else:
        print("    1. Restart Codex Desktop so it reloads ~/.codex/config.toml")
        print("    2. If Codex cannot connect but Terminal can, rerun with --transport http")
        print("    3. Ask Codex to list Resolve tools or read the current project name")
    print()
