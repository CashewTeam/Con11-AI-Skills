# convertPositionTo | PICO Spatial SDK

core / com.pico.spatial.core.ecs / Entity / convertPositionTo 
# convertPositionTo
```kotlin
@MainThread
...
```
entityB.components.get<TransformComponent>()?.position = entityA.convertPositionTo(positionA,entityB.getParent())
```
#### Return
The converted position relative to target entity. 
#### Parameters
position 
The position relative to the current entity. 
target Entity 
The target entity to which the position will be converted. If null, the position will be converted relative to the  com.pico.spatial.core.container.SpatialContainer  where the current entity is placed.