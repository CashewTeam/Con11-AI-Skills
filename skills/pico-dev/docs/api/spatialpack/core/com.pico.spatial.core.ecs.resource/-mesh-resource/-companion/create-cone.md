# createCone | PICO Spatial SDK

core / com.pico.spatial.core.ecs.resource / MeshResource / Companion / createCone 
# createCone
```kotlin
@JvmStatic
```fun  createCone ( height :  Float ,  radius :  Float ) :  MeshResource 
Synchronously creates a cone mesh resource. 
Bounding Box Specification: The bounding box of the generated cone has a fixed center at the origin {0.0f, 0.0f, 0.0f} (local space). Its half-extents are defined as follows: 
- 
...
#### Return
A  MeshResource  object representing the generated cone. 
#### Parameters
height 
The height of the cone, measured in meters (m). Must be greater than or equal to  0 . 
radius 
The radius of the cone, measured in meters (m). Must be greater than or equal to  0 . 
#### Throws
