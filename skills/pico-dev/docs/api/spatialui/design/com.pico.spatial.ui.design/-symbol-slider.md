# SymbolSlider | PICO Spatial SDK

ui:design / com.pico.spatial.ui.design / SymbolSlider 
# SymbolSlider
```kotlin
@Composable
...
#### Parameters
value 
current value of the Slider. Should be in  valueRange , defaults from 0.0 to 1.0. 
on Value Change 
...
icon 
left symbol icon, volume for example. Displayed only when the SliderSpec is at max and regular. 
        ...
        icon = {
            Icon(
                painter =
                    painterResource(
                        id =
                            if (sliderValue > 0) R.drawable.ic_sample_volume_on
                            else R.drawable.ic_sample_volume_off
                    ),
