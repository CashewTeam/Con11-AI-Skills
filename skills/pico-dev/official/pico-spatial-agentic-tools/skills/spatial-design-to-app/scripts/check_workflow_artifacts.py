#!/usr/bin/env python3
"""Validate scratch workflow artifacts for spatial-design-to-app.
...
- input_envelope.json
- design_escalation_receipt.json (required for no-visual intent/product/hybrid inputs)
...
def _has_visual_asset(input_envelope: dict[str, Any]) -> bool:
    sources = input_envelope.get("input_sources")
    if not isinstance(sources, list):
        return False
    return any(
        isinstance(source, dict) and source.get("type") in VISUAL_SOURCE_TYPES
        for source in sources
    )


def _requires_design_escalation(input_envelope: dict[str, Any]) -> bool:
    input_mode = input_envelope.get("input_mode")
    if input_mode in {"intent_only", "product_doc"}:
        return True
    return input_mode == "hybrid" and not _has_visual_asset(input_envelope)
...
def validate_visual_content_contract(contract: Any, label: str) -> dict[str, Any]:
    ...
    search_pill_value = sidebar.get("search_pill")
    if search_pill_value is not None:
        search_pill = _ensure_dict(search_pill_value, f"{label}.sidebar.search_pill")
        if search_pill.get("present") is not False:
    ...
    chips_value = sidebar.get("chips")
    if chips_value is not None:
        chips = _ensure_dict(chips_value, f"{label}.sidebar.chips")
        if chips.get("present") is not False:
        ...
        for key in ("active_chips_have_close_icon", "recommendation_chips_may_have_leading_icon"):
            if key in chips and not isinstance(chips[key], bool):
                raise SystemExit(
                    f"[artifact-check] {label}.sidebar.chips.{key} must be a boolean"
                )
...
def _confidence_number(value: Any, label: str) -> float:
    if not isinstance(value, (int, float)):
        raise SystemExit(f"[artifact-check] {label} must be a number")
    return float(value)


def _trace_mentions_design_package_source(normalized_spec: dict[str, Any]) -> bool:
    trace = _ensure_list(normalized_spec.get("evidence_trace"), "Normalized Spatial Spec.evidence_trace")
    for item in trace:
        if not isinstance(item, dict):
            continue
        haystack = " ".join(str(value) for value in item.values())
        if any(source in haystack for source in DESIGN_PACKAGE_TRACE_SOURCES):
            return True
    return False
...
def validate_design_escalation_receipt(
    ...
    required = _requires_design_escalation(input_envelope)
    if not required:
        ...
        if receipt.get("gate_required") is not False:
            raise SystemExit(
                "[artifact-check] design_escalation_receipt.json must set gate_required=false "
                "when visual input makes the designer gate not required"
            )
    ...
    if not receipt_path.exists():
        raise SystemExit(
            f"[artifact-check] Missing required file: {receipt_path}. "
            "No-visual inputs require design_escalation_receipt.json before Phase 3."
        )
    ...
    if receipt["input_mode"] != input_envelope.get("input_mode"):
        raise SystemExit("[artifact-check] Design Escalation Receipt.input_mode must match Input Envelope.input_mode")
    if receipt["gate_required"] is not True:
        raise SystemExit("[artifact-check] Design Escalation Receipt.gate_required must be true for no-visual inputs")
    if receipt["visual_asset_present"] != _has_visual_asset(input_envelope):
        raise SystemExit(
            "[artifact-check] Design Escalation Receipt.visual_asset_present must match Input Envelope.input_sources"
        )

    status = receipt["status"]
    if status == "designer_passed":
        pre_gates = _ensure_dict(receipt.get("pre_gates"), "Design Escalation Receipt.pre_gates")
        _require_keys(
            pre_gates,
            ["designStatus", "downstreamAppGenerationAllowed", "mainThreadAcceptanceRecorded"],
            "Design Escalation Receipt.pre_gates",
        )
        if pre_gates["designStatus"] != "ready_for_design_delivery":
            raise SystemExit("[artifact-check] designer_passed requires designStatus=ready_for_design_delivery")
        if pre_gates["downstreamAppGenerationAllowed"] != "yes":
            raise SystemExit("[artifact-check] designer_passed requires downstreamAppGenerationAllowed=yes")
        if pre_gates["mainThreadAcceptanceRecorded"] is not True:
            raise SystemExit("[artifact-check] designer_passed requires mainThreadAcceptanceRecorded=true")
        if receipt["bridge_allowed"] is not True or receipt["adapter_extraction"] != "design_package_bridge":
            raise SystemExit(
                "[artifact-check] designer_passed requires bridge_allowed=true and adapter_extraction=design_package_bridge"
            )
        confidence = _ensure_dict(evidence_packet.get("confidence"), "Evidence Packet.confidence")
        layout_confidence = _confidence_number(confidence.get("layout"), "Evidence Packet.confidence.layout")
        if layout_confidence < 0.8:
            raise SystemExit(
                "[artifact-check] designer_passed requires Evidence Packet.confidence.layout >= 0.8"
            )
        if not _trace_mentions_design_package_source(normalized_spec):
            raise SystemExit(
                "[artifact-check] designer_passed requires Normalized Spatial Spec.evidence_trace "
                "to cite a design package source"
            )
    elif status == "fallback_accepted":
        raise SystemExit(
            "[artifact-check] no-visual app generation must complete pico-spatial-app-designer "
            "and record status=designer_passed before Phase 3; shallow_text_extraction is not a valid fallback"
        )
    else:
        raise SystemExit(
            "[artifact-check] Design Escalation Receipt.status must be designer_passed "
            f"for no-visual inputs, got {status!r}"
        )
...
def main() -> int:
    ...
    assumptions = _ensure_list(_load_json(assumption_path), "Assumption Ledger")
