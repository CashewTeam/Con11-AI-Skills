...
- See `spatial-anchor.md` for the authority status of specific
...
shapes & code samples: `spatial-anchor.md`.
...
3. **Multiple panels that need independent open / close lifecycles, or
   ...
   manifest. See `window-container.md` § DefaultWindowContainer
...
## Default root vs secondary containers (NOT a "mixed root")

The container decision picks exactly **one default root** — the launcher entry
configured in `AndroidManifest.xml` and guarded at runtime by
`SUISpatialContainerManager.checkDefault()` (registering two defaults throws
"Default… registered already"). But one app may **also** declare secondary,
runtime-opened containers, and that is legal and common:

| Kind | DSL | Count | Opened by | Manifest? |
|---|---|---|---|---|
| Default root | `DefaultWindowContainer {}` / `DefaultStage {}` | exactly 1 | PICO OS launcher on start | yes |
| Secondary window | `WindowContainer(id=…) {}` | many | `openWindowContainer(id)` | no |
| Secondary stage | `Stage(id=…) {}` | many | `openStage(id, style)` | no |

So `DefaultWindowContainer { … }` (shared-space default) **plus** `Stage(id=…)`
entries opened on demand is a **valid single-root app** — shared-space window
launcher that opens an immersive stage when the user asks for it. Verified
against `SpatialAppSample/stagerendering/Main.kt` (one `DefaultWindowContainer`
control panel + three `Stage(id=…)` scenes). This is **NOT** an illegal mixed
root. The only illegal case is two coexisting **default** roots
(`DefaultWindowContainer` + `DefaultStage`).

When validating, judge the **default** root only. Do not flag secondary
`WindowContainer(id=…)` / `Stage(id=…)` as extra roots. Stage-only APIs
(anchor / mesh / `scene.rayCast`) are legal inside a secondary `Stage(id=…)`
block even when the default root is a WindowContainer.

## Responsibility boundaries (Stage vs WindowContainer surfaces)

Keep each surface type to its own toolkit; do not carry one container's idioms
into the other.

| Need | Use | Do NOT use |
|---|---|---|
| Immersive 3D scene / free spatial content | `Stage` (default or secondary) hosting `SpatialView` + ECS `Entity` | `Box`/`Column`/`Canvas` as a fake scene |
| 2D controls inside a Stage | `AttachmentPanel(id){}` on an ECS anchor, positioned in meters | `TabBar` / `Toolbar` / `Subwindow` / bare Compose overlay |
| Edge-pinned nav / tool strip in a window | `TabBar` / `Toolbar` (sibling of main panel) | `AttachmentPanel`; `Box.align(...)` overlay |
| Persistent auxiliary panel in a window app | `Subwindow {}` or `WindowContainer(id=…)` | `Stage`; a second `DefaultWindowContainer` |
| Anchors / env mesh / ray cast / skybox | `Stage` (Full Space) | any `WindowContainer` |

`Subwindow` / `TabBar` / `Toolbar` are **WindowContainer-side** fittings and must
not appear as the way to place 2D UI inside a Stage; inside a Stage, 2D UI is an
`AttachmentPanel` on an anchor (see `stage.md` → "Stage content model").

## Container migration (follow-up: changing the root of an existing module)

A container decision covers the initial choice; a follow-up may need to migrate
an existing root (e.g. Stage → WindowContainer, or the reverse). Migration is an
architecture change, not a visual tweak.

**Rule: update the contract first, then the code.** Never migrate a container to
satisfy a compile error or a short-term visual fix while the contract, manifest,
entry chain, state, coordinates, and verification artifacts drift out of sync.
Re-run Phase 4 (container decision) and rewrite the `Spatial Layout Contract`
(`container` / `container_reason`, and any `stage_content_strategy` /
`window_chrome_ornaments` implied by the new root) before touching Kotlin.

**Do not over-spatialize.** A non-immersive app must not be forced into a Stage;
enter a Stage only when immersive expression (anchors / env mesh / free 3D /
skybox) is genuinely required. Likewise, do not cram a genuinely immersive need
into a WindowContainer with workarounds.

### Stage → WindowContainer migration checklist

Update every item, in order, and re-verify:

1. **`Main.kt` / `mainApp`** — replace `DefaultStage { … }` with
   `DefaultWindowContainer { … }`; move immersive scene content out or behind a
   secondary `Stage(id=…)` if still needed on demand.
2. **Manifest metadata** — swap `pico.spatial.stage.*` for
   `pico.spatial.windowcontainer.*` (`id`, `style`, `defaultsize`, …) on the
   launcher Activity. See `manifest-and-entry.md`.
3. **`SpatialView` / ECS coordinates** — 3D content that stays must move to a
   secondary `Stage(id=…)`; window content is 2D Compose. Do not leave metric
   (meter-space) transforms driving a 2D window layout.
4. **2D UI ornaments** — re-express Stage `AttachmentPanel` controls as
   window-level `TabBar` / `Toolbar` / `Subwindow` siblings of the main panel.
5. **Runtime launch / capture** — re-run the launch + capture verification so
   manifest, entry chain, and rendered surface are proven consistent, not assumed.

The same checklist runs in reverse for WindowContainer → Stage (window fittings
become `AttachmentPanel`s on anchors; `windowcontainer.*` meta becomes
`stage.*`; 2D page content becomes `SpatialView`-hosted scene content per the
declared `stage_content_strategy`).
