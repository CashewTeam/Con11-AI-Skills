...
This file defines the stable end-to-end workflow for `spatial-design-to-app`.
...
- `.scratch/design_escalation_receipt.json` ← **conditional** for no-visual inputs that trigger Phase 1.5a
...
```bash
# From the spatial-design-to-app skill root:
...
- Phase 1.5a (Design escalation gate) → `Design Escalation Receipt` when no visual asset exists
...
6. **Design Escalation Receipt** (`design_escalation_receipt.json`) when
   `input_mode ∈ {intent_only, product_doc}` or `input_mode=hybrid` has no
   Figma URL / screenshot / mockup / image source.

If one of these is missing, the workflow is incomplete.

> On the no-visual escalation path (see Phase 1.5a — Design escalation gate),
> artifacts 2–4 are sourced from the `pico-spatial-app-designer` design package
> via `design-package-bridge.md`, not from shallow text extraction.
...
both visual and interaction semantics for elements that actually exist in the
reference. Sidebar child structures such as `search_pill` and `chips` are
declaration-driven: omit them, or set `present=false`, when the reference has no
such element. If a screenshot element is recognized as a search input,
`search_pill.interaction_role` MUST be `search_input` and `preferred_component`
MUST be SpatialUI `SearchField`; emitting a static `Row + Icon + Text` for that
element is a Phase-6 implementation error.
...
This gate is a Phase 1.5 pre-step for no-visual requests. It does not add an
`input_mode`, does not alter `../adapters/_registry.json`, and does not add any
top-level schema field; it only decides "design first, then extract".

- **Gate inputs (decision):** `input_mode` plus whether any visual asset exists
  in `input_sources[]`. The gate fires when `input_mode ∈ { intent_only,
  product_doc }`, or `input_mode = hybrid` with no Figma URL and no
  screenshot/mockup. When a Figma URL, screenshot, or mockup is present the gate
  does NOT fire and the run continues on the existing flow.
- **When it fires:** delegate to `pico-spatial-app-designer` to produce a design
  package before evidence extraction. The design package comprises 6
  `review/*.md` role docs (`pm-requirement-spec` / `uxr-research-report` /
  `interaction-spatial-spec` / `visual-system-spec` / `design-critique-report` /
  `preview-qa-report`) plus `preview.html`.
- **Three pre-gates (all required):** `designStatus = ready_for_design_delivery`,
  `downstreamAppGenerationAllowed = yes`, and a recorded main-thread acceptance.
  Sources for these gates are defined in `design-package-bridge.md` §A.1.
- **Bridge output location:** once the three gates pass, the selected adapter
  (`intent-adapter` / `prd-adapter`) extracts the design-package facts per
  `design-package-bridge.md` into the existing three-artifact set
  (`evidence_packet.json` / `normalized_spatial_spec.json` /
  `assumption_ledger.json`) under `<target>/.scratch/`. The bridge only reads the
  design package; it never writes back to `review/*.md` or `preview.html`.
- **Receipt:** persist `<target>/.scratch/design_escalation_receipt.json` before
  Phase 3. `status=designer_passed` requires all three pre-gates plus
  `bridge_allowed=true` / `adapter_extraction=design_package_bridge`.
  `status=fallback_accepted` / `adapter_extraction=shallow_text_extraction` is
  not a valid no-visual app-generation path.
- **Blocked state:** if `pico-spatial-app-designer` is unavailable, the user
  declines a full design pass, or any of the three gates fails, do NOT run the
  bridge and do NOT fall back to shallow extraction. Stop before Phase 3 and
  report BLOCKED with the missing designer deliverables/gates.
- **Contract preservation:** this gate does not modify the 7-phase contract; the
  final container / window model decision is still made in Phase 4.
...
- consult `container-decision.md` (now contains the **container ×
  feature legality** table)
- consult `window-model-decision.md` (now contains the **Subwindow
...
- product-specific new project request: hand the resolved container contract to
  `spatial-app-onboarding`, let onboarding scaffold the complete base project,
  then resume this workflow against the runnable project handoff
...
- `Container Decision` → `rejected_near.{alternative, rejection_reason}` and
  ...
  - both `rejection_reason` values must cite either a `facts.<key>` value or a
    row of the legality table in `container-decision.md`
...
And, when the design has any inset / padding / gap:

- `root_fill` (`fill_window` vs `padded_card`) — explicit, never inferred at codegen
- `spacing_ownership[]` — every meaningful inset/padding/gap mapped to its owning node

And, when `container` is any `STAGE_*` mode:

- `stage_content_strategy` — one of `ecs_runtime` / `editor_bundle` / `explicit_fallback` (a Stage must not resolve to a flat Compose page)
- for each 2D control in the stage: its `AttachmentPanel` anchor and metric (meters) position
...
- exactly one **default** root (`DefaultWindowContainer` / `DefaultStage`); secondary `WindowContainer(id=…)` / `Stage(id=…)` are legal and are NOT extra roots
- Stage-only APIs stay in Stage flows (default Stage, or a secondary `Stage(id=…)` block)
- a `STAGE_*` root actually hosts spatial content (`SpatialView` / ECS entities), not just `Box` / `Column` / `Canvas`; 2D controls use `AttachmentPanel`, not bare overlays
- any container migration updated the contract first (container decision re-run), then Main.kt, manifest, coordinates, ornaments, and runtime launch/capture
...
- If the design has any inset/padding/gap, did I set `root_fill` explicitly and map every gap to an owner in `spacing_ownership`?
- If the container is a `STAGE_*` mode, did I declare a `stage_content_strategy` (not a flat Compose page) and give each 2D control an `AttachmentPanel` anchor + metric position?
...
```json
{
  ...
  "target_module": null,
  "input_sources": [{ "type": "figma_url", "trust_level": "high" }]
}
```
...
```json
{
  ...
  "generation_mode": "new_project",
  "target_module": null,
  "input_sources": [{ "type": "prd_markdown", "trust_level": "high" }]
}
```
