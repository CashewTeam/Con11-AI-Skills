...
| Symptom | Fix |
|---|---|
| `Unresolved reference: <Component>` | First check `spatial-ui-components.md`; the component may be invented or unsupported by the current SDK. |
...
| `IllegalStateException: not in Full Space` | Stage-only API was called from WindowContainer; revisit `spatial-anchor.md` and the Phase-4 container decision. |
| `mainApp` not invoked / blank launch after install | Check `Application.launch(::mainApp)`, `SpatialLaunchActivity`, and manifest entry wiring in `manifest-and-entry.md`. |
