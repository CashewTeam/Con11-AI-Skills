...
| `root_fill` | enum | when the design has any inset/padding/gap | `fill_window` (root shell fills the window, spacing lives on inner containers) or `padded_card` (root shell is an inset card with its own outer padding). Must be explicit — never inferred at codegen time. |
| `spacing_ownership` | object[] | when the design has any inset/padding/gap | One entry per meaningful inset/padding/gap, mapping the spacing to the node that owns it. See "Spacing ownership & root fill" below. |
| `stage_content_strategy` | enum | when `container` is any `STAGE_*` mode | `ecs_runtime` / `editor_bundle` / `explicit_fallback`. Declares how the immersive scene is built so a Stage never resolves to a flat Compose page. See `stage.md` → "Stage content model". |
...
When the design has any inset, padding, or gap, spacing is **not** a free-floating
number — every meaningful gap belongs to exactly one node. Producing code without
deciding ownership is what causes double padding, missing edge insets, or a card
that should fill the window but ends up floating with a margin.

**Rule 1 — `root_fill` is mandatory when spacing exists.** State whether the root
shell fills the window or is an inset card. Never let codegen guess.

**Rule 2 — emit `spacing_ownership` for every inset/padding/gap.** Each entry names
the spacing, its value, and the single node that applies it:

```json
"spacing_ownership": [
  { "id": "root_edge_inset", "value_dp": 24, "owner": "content_column", "kind": "padding",
    "note": "root fills window; inset lives on the inner content column, not the root" },
  { "id": "card_gap", "value_dp": 12, "owner": "content_column", "kind": "vertical_gap" },
  { "id": "card_inner_padding", "value_dp": 16, "owner": "content_card", "kind": "padding" }
]
```

`kind` ∈ `padding` | `horizontal_gap` | `vertical_gap` | `inset` | `margin`.

**Rule 3 — the scanner must distinguish two structures.** They look similar in a
screenshot but have different ownership, and must not be collapsed into one:

| Structure | Root shell | Where spacing lives | `root_fill` |
|---|---|---|---|
| **root fill + internal inset** | fills the whole window (edge-to-edge background/surface) | inner content container carries the edge inset + gaps | `fill_window` |
| **outer padding card** | an inset card that does NOT reach the window edges | root card carries its own outer padding/margin; children sit inside | `padded_card` |

Getting this wrong produces the classic bugs: putting the edge inset on a
`fill_window` root (background stops short of the edges), or forgetting the outer
margin on a `padded_card` root (card bleeds to the window edge). Decide the
structure explicitly, then attribute each gap in `spacing_ownership` accordingly.
